...
export async function POST(request: NextRequest) {
  try {
    const { customerEmail, customerName, orderNumber, newStatus, statusNote, items } =
      await request.json();
    ...
    if (newStatus === 'delivered' && items && items.length > 0) {
      ...
      await sendProductReviewRequestEmail(customerEmail, customerName, orderNumber, items).catch(
        (e) => console.error(e)
      );
    }
