...
export function useGetNotifications() {
  ...
  useEffect(() => {
    ...
    const q = query(collection(db, 'notifications'), where('userId', 'in', targetIds));
...
export function useNotificationMutations() {
  ...
  const createNotification = useCallback(
    async (data: Omit<NotificationRecord, 'id' | 'createdAt' | 'isUnRead'>) => {
      setLoading(true);
      try {
  ...
  const markAllAsRead = useCallback(async (notificationsToUpdate: NotificationRecord[]) => {
    setLoading(true);
    try {
