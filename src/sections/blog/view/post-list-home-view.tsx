...
import { usePosts } from 'src/hooks/firebase';
...
export function PostListHomeView() {
  const { posts, loading } = usePosts({ publish: true });
  const [sortBy, setSortBy] = useState('latest');

  const dataFiltered = applyFilter({ inputData: posts, sortBy });

  const renderNoData = () => <EmptyContent title="No Blog Posts Yet" filled sx={{ py: 10 }} />;
  ...
  return (
    <Container sx={{ mb: 10 }}>
      ...
      >
        <PostSearch redirectPath={(id: string) => paths.blog.details(id)} />
