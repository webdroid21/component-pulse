...
} from 'src/types/training-module';
...
export function TrainingModuleForm({ currentModule }: Props) {
  ...
  const {
    uploadMaterials,
    loading: uploadingMats,
    progress: matsProgress,
  } = useTrainingMaterialsUpload();
  ...
  const handleRemoveMaterial = useCallback(
    (inputFile: File | string) => {
      setMaterialFiles((prev) =>
        prev.filter((file) => {
          if (file instanceof File && inputFile instanceof File)
            return file.name !== inputFile.name;
          if (!(file instanceof File) && typeof inputFile === 'string')
            return file.url !== inputFile;
          return true;
        })
      );
      setValue(
